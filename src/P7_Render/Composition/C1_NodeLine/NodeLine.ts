 ...
 * 引导线竖线相对行左缘的内缩量
 *
 * 引导线（C2_Tree/TreeGuides 画）与转角方块（本文件的 GET_ParentX 定位）共用这一个基准 ——
 * 两处若各写各的数值，方块就会偏离竖线（此前 base=8 与 GUIDE_INSET=4 并存，方块偏右 4px）。
 * 放在 C1 是因为 C2 依赖 C1，反过来不成立。
 */
export const GUIDE_INSET = 4;
