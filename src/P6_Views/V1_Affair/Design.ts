 ...
 */
function openTextTreeImport(plugin: SeqtkPlugin, text: string): void {
...
export class DesignView extends ReactViewBase {
    ...
    public editSubtreeAsText(nodeId: string): void {
        ...
        new TextTreeImportModal(this.app, {
            ...
            notice: (roots) => {
                ...
                const plan = PLAN_TextTreeEdit(this.pipe, nodeId, roots[0]);
                const lines = [
    ...
    public editFrameworkContentAsText(frameworkId: string): void {
        ...
        new TextTreeImportModal(this.app, {
            ...
            initialText: text,
            notice: (roots) => {
                const plan = PLAN_TextTreeEdit(this.pipe, frameworkId, asFakeRoot(roots));
                const lines = [
    ...
    /** 执行框架内容回写（多根） */
    private async applyFrameworkContentEdit(frameworkId: string, roots: TextTreeNode[]): Promise<void> {
        const framework = this.pipe.GET_Node(frameworkId);
        if (!framework) return;
        const result = await APPLY_TextTreeEdit(this.pipe, frameworkId, {
            kind: framework.kind,
            state: framework.state ?? 'plan',
            desc: framework.desc,
            line: 0,
    ...
    private async applySubtreeEdit(nodeId: string, roots: TextTreeNode[]): Promise<void> {
        if (roots.length !== 1) {
            new Notice('根节点必须恰好一个，未做任何修改');
            return;
        }
        const result = await APPLY_TextTreeEdit(this.pipe, nodeId, roots[0]);
        if (result.error) {
            new Notice(
                `回写中断：已更新 ${result.updated}、新增 ${result.created}、删除 ${result.removed}；失败：${result.error}`,
            );
