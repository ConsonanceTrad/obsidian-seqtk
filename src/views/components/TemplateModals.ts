/**
 * TemplateModals — 模板相关共享模态框（DesignView 右键「创建/使用模板」与 TemplateView 共用）
 *
 * - SelectFrameworkModal：选择目标框架（模板框架 / 目标框架），原 TemplateView 私有类迁移共享
...
/** 选择框架模态框（原 TemplateView 私有类，迁移至此共享） */
 ...
 */
export function listTemplateUnits(nodeCache: NodeCache): TemplateUnitEntry[] {
  const units: TemplateUnitEntry[] = [];
  for (const f of nodeCache.getByKind('framework-template')) {
    for (const child of nodeCache.getChildren(f.nodeId)) {
      if (!child.data) continue;
      units.push({
        framework: { nodeId: f.nodeId, desc: f.data.desc },
        unit: { nodeId: child.nodeId, data: child.data },
        subtreeSize: 1 + nodeCache.collectDescendants(child.nodeId).length,
      });
    }
  }
  return units;
...
export class TemplateUnitSelectModal extends Modal {
  ...
  private renderSubtreePreview(container: HTMLElement, parentNodeId: string, depth: number): void {
    const children = this.nodeCache.getChildren(parentNodeId);
  ...
  private hasChildren(nodeId: string): boolean {
    return this.nodeCache.getChildren(nodeId).some((c) => !!c.data);
  }
