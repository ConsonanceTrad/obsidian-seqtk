...
| 模板模式 | 模板库管理：左栏模板框架树负责选框架与增删（可像事务设计那样整体委托到侧栏），右栏上预览下编辑、以文本直接编辑选中框架的内容（实时解析预览与合法性校验），再把模板应用到目标框架 | 可用 |
...
- **委托左栏的树** —— 把左栏的树（事务设计的框架树、模板模式的模板框架树）委托到侧栏（借用中控台侧栏，或独立视图），两侧的展开与选中保持同步。委托全局互斥：同一时刻只有一份，新委托会先把旧的释放并交还其来源视图。
