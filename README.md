...
| 打开日志阅览 | 日志阅览（规划中） |
| 打开智能协作 | 智能协作（规划中） |
| 委托框架树到侧栏（独立视图） | 把框架树放到独立侧栏视图 |
| 重建查询缓存 | 丢弃并从节点文档重建查询缓存 |
...
- **数据根文件夹**：插件管理的节点所在位置。
- **默认排序**：按创建时间、修改时间、名称或状态，以及升降序。
- **状态传播规则**：父状态变化时向下传播、子节点全部达标时向上聚合；规则可增删改，也可以完全不配。
- **归档 / 删除策略**：处理父节点时后代如何处理，以及归档、删除各自的确认级别。
- **左侧栏入口**：是否在事务设计里显示「全部事务」总览入口。
- **委托落点**：框架树委托到中控台侧栏，还是开出独立视图。
- **界面记忆**：左栏宽度、两栏的展开状态、滚动位置、上次选中的框架都会记住，重开库时恢复。

---

## 边界与现状

- 这是一套**面向长期复杂任务的工作方法**，不是通用的待办清单工具。用不上层级、证据、留痕的短任务，用它反而多花钱。
